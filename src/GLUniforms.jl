...
# Needs to be 
# Needs to be 
gluniform(location::Integer, x::AbstractRGB) 		     = gluniform(location, reinterpret(Vector3{eltype(x)}, [x])[1])
gluniform(location::Integer, x::AbstractAlphaColorValue) = gluniform(location, reinterpret(Vector4{eltype(x)}, [x])[1])
