...
mutable struct FrameBuffer{ElementTypes, Internal}
    ...
    function FrameBuffer(fb_size::Tuple{<: Integer, <: Integer}, texture_types::NTuple{N, Any}) where N
        ...
        depth_types = collect(Iterators.filter(x-> x <: DepthFormat, texture_types))
        other_types = collect(Iterators.filter(x -> !(x <: DepthFormat) && hasmethod(length, (x,)), texture_types))
        invalid_types = collect(Iterators.filter(x -> (x ∉ depth_types && x ∉ other_types), texture_types))
